...
void COverlay::ExpandLeft(int nPixels)
{
	for (int i = 0; i < clBoxList.cnt; i++)
	{
		clBoxList.r[i].left  -= nPixels * theApp.m_Vision.m_dZoomRate; // 왼쪽 경계를 왼쪽으로 확장
	}
}

void COverlay::ExpandRight(int nPixels)
{
	for (int i = 0; i < clBoxList.cnt; i++) {
		clBoxList.r[i].right += nPixels * theApp.m_Vision.m_dZoomRate;
	}
}

void COverlay::ExpandTop(int nPixels)
{
	for (int i = 0; i < clBoxList.cnt; i++) {
		clBoxList.r[i].top -= nPixels * theApp.m_Vision.m_dZoomRate;
	}
}

void COverlay::ExpandBottom(int nPixels)
{
	for (int i = 0; i < clBoxList.cnt; i++) {
		clBoxList.r[i].bottom += nPixels* theApp.m_Vision.m_dZoomRate;
	}
}