...
MIL_INT MFTYPE ProcGrab(MIL_INT HookType, MIL_ID HookId, void* HookDataPtr)
{
	...
	CProject08Dlg* pDlg = (CProject08Dlg*)theApp.m_pMainWnd;
	if (!pDlg)
		return -1;  
...
void CVision::DefineModel()
{
	// 모델 정의
	MmodAlloc(MilSystem, M_GEOMETRIC, M_DEFAULT, &MilSearchContext);
	MmodDefine(MilSearchContext, M_IMAGE, MilImageZoom[0], 100, 100, 200, 200);
} 

void CVision::PreprocessModel()
{
	// 모델 전처리
	MmodPreprocess(MilSearchContext, M_DEFAULT);
}

void CVision::FindModel()
{
	// 모델 검색
	MmodAllocResult(MilSystem, M_DEFAULT, &MilSearchResult);
	MmodFind(MilSearchContext, MilImageZoom[0], MilSearchResult);
}